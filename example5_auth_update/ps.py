...
def update_html(your_name_new):

		your_name_prev = ''
		with open('welcome_page.html') as f:
			while True:
				line_f = f.readline()
				#print "lein:",line_f
				if 'name="your_name"' in line_f:
		        		your_name_prev = line_f
					#print your_name_prev
					break
	
		#your_name_new = 'Mehmood'
		fin = open("welcome_page.html", "rt")
	
		data = fin.read()
		data = data.replace(your_name_prev, "<input type=\"text\" name=\"your_name\" value=\""+your_name_new+"\"/>\n")
		fin.close()
	
		fin = open("welcome_page.html", "wt")
		fin.write(data)
		fin.close()
...
class myHandler(BaseHTTPRequestHandler):
	...
	def do_POST(self):
		...
		if self.path=="/send":
			...
			update_html(form["your_name"].value)
			self.send_response(302)
			self.send_header('Location','/welcome_page.html')
			self.end_headers()
			return	
